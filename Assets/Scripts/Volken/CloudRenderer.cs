...
public class CloudRenderer : MonoBehaviour
{
    ...
    /*
    private CommandBuffer cloudCB;

    private RenderTargetIdentifier cameraTarget;
    private void Awake()
    {
        // 创建 CommandBuffer
        cloudCB = new CommandBuffer();
        cloudCB.name = "Volumetric Clouds";

        // 选择注入点 —— 强烈推荐 AfterSkybox 或 AfterDepth
        // AfterSkybox：天空盒之后，opaque 几何体之后，transparent 之前（最常见完美位置）
        // AfterDepth：深度纹理写完后，也很好
        cam.AddCommandBuffer(CameraEvent.AfterEverything, cloudCB);
    ...
    private void Update()
    {
        // 每帧重建 CommandBuffer（因为 source 是相机当前颜色缓冲，需要动态获取）
    ...
    private void RebuildCommandBuffer()
    {
        ...
        SetDynamicProperties();  // 每帧更新风、光照等

        // 下面所有 Blit 命令加入 CB
        cloudCB.Blit(FarCameraScript.farDepthTex, combinedDepthTex, mat, mat.FindPass("NearDepth"));

        cloudCB.Blit(combinedDepthTex, lowResDepthTex, mat, mat.FindPass("DownsampleDepth"));

        cloudCB.SetGlobalTexture("DepthTex", lowResDepthTex);
        cloudCB.SetGlobalTexture("HistoryTex", cloudHistoryTex);
        cloudCB.SetGlobalTexture("HistoryDepthTex", cloudHistoryDepthTex);
        cloudCB.Blit(null, cloudTex, mat, mat.FindPass("Clouds"));

        cloudCB.Blit(cloudTex, cloudHistoryTex);
        cloudCB.Blit(lowResDepthTex, cloudHistoryDepthTex);

        cloudCB.SetGlobalTexture("CombinedDepthTex", combinedDepthTex);
        cloudCB.SetGlobalTexture("LowResDepthTex", lowResDepthTex);
        cloudCB.SetGlobalInt("isNativeRes", (cloudTex.width == cam.pixelWidth && cloudTex.height == cam.pixelHeight) ? 1 : 0);
        cloudCB.Blit(cloudTex, upscaledCloudTex, mat, mat.FindPass("Upscale"));

        cloudCB.SetGlobalTexture("UpscaledCloudTex", upscaledCloudTex);
        cloudCB.SetGlobalTexture("SceneDepthTex", combinedDepthTex);

        // 最后一步：把云复合到相机当前颜色缓冲（即屏幕）
        cloudCB.Blit(BuiltinRenderTextureType.CurrentActive, cameraTarget, mat, mat.FindPass("Composite"));
    }*/
    [ImageEffectOpaque] 
